...
void TwoFunctions(  
    ...
    bool useArgus= 1, float fixCoef = -63.5, float fixThres = 5.29,// 
...
/* 35 */"#Lambda_{c}^{-}p#pi^{+}",
    ...
    RooRealVar *mean1;
    RooRealVar *mean2;
        mean1 = new RooRealVar("mean1", "", 5.279);
        mean2 = new RooRealVar("mean2", "", 5.28,5.277,5.283);
    RooRealVar *sigma1;
    RooRealVar *sigma2;
        sigma1 = new RooRealVar("sigma1", "", 0.00299);
        sigma2 = new RooRealVar("sigma2", "", 0.0023,0.,0.2);
    RooRealVar *alpha   = new RooRealVar("alpha", "", .8,.7,.9); 
    RooRealVar *n       = new RooRealVar("n","",3,1,10);
    RooGaussian sig1("sig1", "", Mbc, *mean1, *sigma1);
    RooCBShape sig2("sig2","",Mbc,*mean2,*sigma2,*alpha,*n);
    RooRealVar frac("frac","",.95,0.,1.);
    ...
    Mbc.setRange("sband1",lowBound,5.26);
