...
void CB_fit(  
    ...
    int myMode = 0, double minProb = 0.1,
...
/* 35 */"#Lambda_{c}^{-}p#pi^{+}",
    ...
    // ------------------------------------------------
    RooRealVar *mean    = new RooRealVar("mean", "", 5.28,5.277,5.283);
    RooRealVar *sigma   = new RooRealVar("sigma", "", 0.0023,0.,0.2);
    RooRealVar *alpha   = new RooRealVar("alpha", "", .8,.7,.9); 
    RooRealVar *n       = new RooRealVar("n","",3,1,10);
    //RooGaussian sig("sig", "", Mbc, *mean, *sigma);
    RooCBShape sig("sig","",Mbc,*mean,*sigma,*alpha,*n);

    RooRealVar *thres   = new RooRealVar("thres","", 5.29);
    RooRealVar *coef    = new RooRealVar("coef","",-90,-1e3,1e3);
    RooArgusBG bkg("bkg","bkg",Mbc,*thres,*coef);

    // A d d  s i g n a l   a n d   b a c k g r o u n d
    // ------------------------------------------------
    RooRealVar nsig("nsig", "", 1e6,0,1e9);
    RooRealVar nbkg("nbkg", "", 4e6,0,4e9);
    ...
    //c1 -> SaveAs(Form("mode_plots/Mode_%02d_CB_%.3f.pdf",myMode,minProb));
