...
void FitModes(  
    int myMode = 0, double minProb = 0.05,
    float upperRange = 5.27, bool lowerLabel = 1,
    ...
    TString fInName = Form("output/Cand_mode_%02d.root",myMode);
    // Variables
...
/* 35 */"#Lambda_{c}^{-}p#pi^{+}",
    ...
    if          ( useGaus  &&  useArgus ){
        ...
        if(lowerLabel){
            sig.paramOn     (frame,    Layout(0.1,0.4,0.4));  
        }else{
        ...
        if(lowerLabel){
            bkg.paramOn     (frame,    Layout(0.1,0.4,0.4));  
        }else{
            bkg.paramOn     (frame,    Layout(0.1,0.4,0.9));
        }
    }
    ...
    c1 -> SaveAs(Form("mode_plots/Mode_%02d_Fit_%.3f.pdf",myMode,minProb));
