...
void CombineFit(  
    ...
    RooDataSet Mbc_data("Mbc_data","", RooArgSet(Mbc, SigProb, decMode, isSignal, isSigAccMiss, isSigAccMissGamma, isSigAccWrongFSP), ImportFromFile(fInName,"B_cand"), Cut(cutString)); 

    Mbc.setBins(nBins);
    RooPlot *frame = Mbc.frame(Title(
    Form("[%02d]   %s   SigProb>%.3f CombinedFit",myMode,fState[myMode],minProb)));
    ...
    RooAddPdf *model = new RooAddPdf("model","sig+bkg",RooArgList(sig,bkg),RooArgList(nsig,nbkg));
    ...
    c1 -> SaveAs(Form("fit_plots/Mode_%02d_Comb_%.3f.pdf",myMode,minProb));

    TCanvas *c2 = new TCanvas("c2", "c2", 800, 600);
    RooPlot *frame2 = Mbc.frame(Title(
    Form("[%02d]   %s   SigProb>%.3f CombinedPull",myMode,fState[myMode],minProb)));
    RooHist *hpull = frame->pullHist();
    frame2->addPlotable(hpull, "PL");
    frame2 -> Draw();
    frame2->SetMinimum(-5);
    frame2->SetMaximum( 5);
    c2 -> SaveAs(Form("fit_plots/Mode_%02d_Comb_%.3f_pull.pdf",myMode,minProb));
