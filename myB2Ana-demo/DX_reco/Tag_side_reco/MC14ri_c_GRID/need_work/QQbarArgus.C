...
void QQbarArgus(  
    ...
    RooRealVar isMixed("isMixed","",-1,1);
    RooRealVar isCharg("isCharg","",-1,1);
    RooRealVar isQQbar("isQQbar","",-1,1);
    ...
    TString cutString("");
    //cutString+=Form("isMixed>.5&&decMode==1.0*%d&&SigProb>%.3f&&isSignal<.5&&isSigAccMiss<.5&&isSigAccMissGamma<.5&&isSigAccWrongFSP<.5",myMode,minProb);
    cutString+=Form("isQQbar>.5&&decMode==1.0*%d&&SigProb>%.3f",myMode,minProb);
    /*
    if(looseMatched){
        cutString+=Form("decMode==1.0*%d&&SigProb>%.3f&&(isSignal>.5||isSigAccMiss>.5||isSigAccMissGamma>.5||isSigAccWrongFSP>.5)",myMode,minProb);
    }else{
        cutString+=Form("decMode==1.0*%d&&SigProb>%.3f",myMode,minProb);
    }
    */
    RooDataSet Mbc_data("Mbc_data","", RooArgSet(isQQbar, Mbc, SigProb, decMode, isSignal, isSigAccMiss, isSigAccMissGamma, isSigAccWrongFSP), ImportFromFile(fInName,"B_cand"), Cut(cutString)); 
...
/* 35 */"#Lambda_{c}^{-}p#pi^{+}",
    ...
    Form("[%02d]   %s   SigProb>%.3f %s ArgusFit",myMode,fState[myMode],minProb,matchStr[looseMatched])));
    ...
    // ------------------------------------------------
    RooRealVar *sigma;
        sigma = new RooRealVar("sigma", "", 0.001618);
    RooRealVar mu("mu","",5.28447);
    RooRealVar lambda("lambda","",0.0046);
    RooRealVar gamma("gamma","",2.376);
    RooRealVar delta("delta","",2.278);
    RooJohnson sig1("sig1","",Mbc,mu,lambda,gamma,delta,5.24);
    RooGaussian sig2("sig2", "", Mbc, mu, *sigma);

    RooRealVar frac("frac","",0.8878);
    ...
    */
    ...
    Form("[%02d]   %s   SigProb>%.3f %s ArgusPull",myMode,fState[myMode],minProb,matchStr[looseMatched])));
    ...
    frame2->SetMinimum(-5);
    frame2->SetMaximum( 5);
    c2 -> SaveAs("qqbarArgus_pull.pdf");
    /*
    if(looseMatched){
        c2 -> SaveAs(Form("looseMatched_plots/Mode_%02d_Johnson_%.3f_pull.pdf",myMode,minProb));
    }else{
        c2 -> SaveAs(Form("plots/Mode_%02d_Johnson_%.3f_pull.pdf",myMode,minProb));
    }
    */
    return;
