...
void ChargBkgFit(  
    ...
    TString cutString("");
    cutString+=Form("isCharg>.5&&decMode==1.0*%d&&SigProb>%.3f&&isSignal<.5&&isSigAccMiss<.5&&isSigAccMissGamma<.5&&isSigAccWrongFSP<.5",myMode,minProb);
    /*
    if(looseMatched){
    ...
    */
    RooDataSet Mbc_data("Mbc_data","", RooArgSet(isCharg, Mbc, SigProb, decMode, isSignal, isSigAccMiss, isSigAccMissGamma, isSigAccWrongFSP), ImportFromFile(fInName,"B_cand"), Cut(cutString)); 
