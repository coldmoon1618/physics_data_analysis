...
void FitMbcBDTcut(  
        int myMode = 0, double minProb = 0.01,
        bool useArgus= 1, float fixCoef = -999, float fixThres = -999,//5.29,// 
        bool useGaus = 1, float fixMean = -999, float fixSigma = -999,
        int nBins = 60, float lowBound = 5.24){

    //TString fInName("Data_cand.root");

    // Variables
    RooRealVar Mbc("Mbc","",lowBound,5.3);
    //RooRealVar decMode("decMode","",0,35);
    //RooRealVar SigProb("SigProb","",0,1);
    //RooDataSet Mbc_data("Mbc_data","",RooArgSet(Mbc, SigProb, decMode),ImportFromFile(fInName,"B_cand"),Cut(Form("decMode==%02f&&SigProb>%.3f",myMode,minProb))); 
...
/*
    ...
    // ------------------------------------------------
    RooRealVar nsig("nsig", "", 3e3, 0, 3e5);
    RooRealVar nbkg("nbkg", "", 1e3, 0, 5e6);
...
*/    
    ...
    frame -> Draw();
    c1 -> SaveAs(Form("temp/%02d.pdf",myMode));
