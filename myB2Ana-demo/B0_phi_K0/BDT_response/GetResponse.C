...
void GetResponse(TString type, TString myMethodList = "" )
{
    ...
    TTree *SigMC    = new TTree("SigMC","");
    SigMC   -> Branch("response", &response);
    TTree *B0bkg    = new TTree("B0bkg","");
    B0bkg   -> Branch("response", &response);
    TTree *BBbkg    = new TTree("BBbkg","");
    BBbkg   -> Branch("response", &response);
    TTree *QQbkg    = new TTree("QQbkg","");
    QQbkg   -> Branch("response", &response);
...
//
    res_BDT -> Branch("Mbc", &candB->Mbc);
    res_BDT -> Branch("deltaE",     &candB->deltaE);
    res_BDT -> Branch("isBphiK",    &candB->isBphiK);
    res_BDT -> Branch("isMixed",    &candB->isMixed);
    res_BDT -> Branch("isBBbar",    &candB->isBBbar);
    res_BDT -> Branch("phi_M",      &cBPhi->M);
    res_BDT -> Branch("phi_cosHel", &cBPhi->cosHel);

    //A_CP stuff
    res_BDT -> Branch("mcDeltaTau",     &candB->mcDeltaTau);
    res_BDT -> Branch("DeltaT",         &candB->DeltaT);
    res_BDT -> Branch("DeltaTErr",      &candB->DeltaTErr);
    res_BDT -> Branch("DeltaTRes",      &candB->DeltaTRes);
    res_BDT -> Branch("MCflvrOtherB",   &candB->MCflvrOtherB);
    res_BDT -> Branch("flvrTag_FBDT",   &candB->flvrTag_FBDT);
    res_BDT -> Branch("isSignal",       &candB->isSignal);

    SigMC   -> Branch("Mbc",        &candB->Mbc);
    SigMC   -> Branch("deltaE",     &candB->deltaE);
    B0bkg   -> Branch("Mbc",        &candB->Mbc);
    B0bkg   -> Branch("deltaE",     &candB->deltaE);
    BBbkg   -> Branch("Mbc",        &candB->Mbc);
    BBbkg   -> Branch("deltaE",     &candB->deltaE);
    QQbkg   -> Branch("Mbc",        &candB->Mbc);
    QQbkg   -> Branch("deltaE",     &candB->deltaE);
    ...
    for (Long64_t ievt=0; ievt<theTree->GetEntries();ievt++) {
        ...
        if(candB->isBphiK>.5){
            SigMC -> Fill();
        }else if(candB->isMixed>.5){
            B0bkg -> Fill();
        }else if(candB->isBBbar>.5){
            BBbkg -> Fill();
        }else{
            QQbkg -> Fill();
        }
