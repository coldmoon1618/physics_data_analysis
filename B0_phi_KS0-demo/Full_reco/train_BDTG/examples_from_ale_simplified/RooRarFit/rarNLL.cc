 ...
 * Package: RooRarFit
 *    File: $Id: rarNLL.cc,v 1.14 2010/06/14 15:46:44 fwilson Exp $
 * Authors: Lei Zhang
 * History:
 * 
...
TArrayD rarNLL::getX(Double_t y)
{
  ...
  Double_t dLimit=(_x2s[_nSteps-1]-_x0s[0])*1e-6;
  if (0==dLimit) dLimit=1e-6;
  if (dLimit>1e-6) dLimit=1e-6;
  TArrayD XX(0);
  if (I.GetSize()>0) {
    XX.Set(1);
    XX[0]=X[I[0]];
  ...
  for (Int_t i=1; i<I.GetSize(); i++) {
    ...
    if (fabs(XX[iXX]-x)<dLimit) { // combine them
      if (_verbose) {
		       ...
		       Double_t a, Double_t b, Double_t c)
{
  ...
  if (a>0) {
    ...
    Double_t dLimit=fabs(x2-x0)*1e-5;
    if (dLimit>1e-5) dLimit=1e-5;
    if (fabs(xmin-x1)<dLimit) {
...
Double_t rarNLL::getLIntegralInverse(Double_t xl, Double_t iVal)
{
  ...
  if (iVal<=0) return -1e10;
  if (iVal>=_tLIntegrals[_nSteps]) return 1e10;
  ...
  } else if ((0!=b)&&(a>-1e-5)&&(fabs(a/b)<100)) { // a ~ 0
