 ...
 * Package: RooRarFit
 *    File: $Id: rarBifurGauss.cc,v 1.4 2007/06/29 08:37:35 zhanglei Exp $
 * Authors: Lei Zhang
 * History:
 * 
...
void rarBifurGauss::init()
{
  ...
  // Config pdf params
  if (0==_parSymLevel) {
    _peak=createAbsReal("peak", "#mu", (x->rar_getMin()+x->rar_getMax())/2,
			x->rar_getMin(), x->rar_getMax(), _x->getUnit());
    _sigL=createAbsReal("sigL", "#sigma_{L}", .1, 0.,
			(x->rar_getMax()-x->rar_getMin())/2,_x->getUnit());
    _sigR=createAbsReal("sigR", "#sigma_{R}", .1, 0.,
			(x->rar_getMax()-x->rar_getMin())/2,_x->getUnit());
  } else {
    createAbsReal("mean", "#mu", (x->rar_getMin()+x->rar_getMax())/2,
		  x->rar_getMin(), x->rar_getMax(), _x->getUnit());
    createAbsReal("rms", "#sigma", .1, 0.,(x->rar_getMax()-x->rar_getMin())/2,
		  _x->getUnit());
