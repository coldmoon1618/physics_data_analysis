 ...
 * Package: RooRarFit
 *    File: $Id: rarConfig.cc,v 1.48 2010/06/15 13:09:58 fwilson Exp $
 * Authors: Lei Zhang
 * History:
 * 
...
#include "RooRarFit/rarAdd.hh"
#include "RooRarFit/rarArgusBG.hh"
#include "RooRarFit/rarDecay.hh"
#include "RooRarFit/rarBasePdf.hh"
#include "RooRarFit/rarBifurGauss.hh"
#include "RooRarFit/rarCBShape.hh"
#include "RooRarFit/rarDatasets.hh"
#include "RooRarFit/rarExp.hh"
#include "RooRarFit/rarGaussModel.hh"
#include "RooRarFit/rarGaussian.hh"
#include "RooRarFit/rarOsipDisc.hh"
#include "RooRarFit/rarGeneric.hh"
#include "RooRarFit/rarHistPdf.hh"
#include "RooRarFit/rarKeys.hh"
#include "RooRarFit/rarMLPdf.hh"
#include "RooRarFit/rarNovosibirsk.hh"
#include "RooRarFit/rarBallack.hh"
#include "RooRarFit/rarPoly.hh"
#include "RooRarFit/rarProd.hh"
#include "RooRarFit/rarSimPdf.hh"
#include "RooRarFit/rarStep.hh"
#include "RooRarFit/rarBinned.hh"
#include "RooRarFit/rarTriGauss.hh"
#include "RooRarFit/rarTwoGauss.hh"
#include "RooRarFit/rarUsrPdf.hh"
#include "RooRarFit/rarCruijff.hh"
#include "RooRarFit/rarMultPdf.hh"
...
TString rarConfig::getFullFileName(TString dir, TString aType, TString name,
                                   ...
                                   TString cfName)
{
  ...
  if (gSystem->AccessPathName(dir)&&gSystem->mkdir(dir)) {
    cout<<"Can not access dir "<<dir<<endl;
    exit(-1);
  }
  // add configFile
  ...
  // add dataset sec
  if (""==dsName) {
    if (getDatasets()) dsName=getDatasets()->getVarSec();
    else dsName=getVarSec();
  }
  fileName+=getFileName(dsName);
  // add master sec
  if (""==msName) msName=getMasterSec();
  fileName+=getFileName(msName);
...
RooAbsArg *rarConfig::createAbsVar(const char *varStr, const char *option)
{
  ...
  if ("nsRooRealVar"==varType) { // new schema
    ...
    // set #bins
    ((RooRealVar*)theVar)->rar_setBins(nBins);
    // set label
    ((RooRealVar*)theVar)->setPlotLabel(myTitle);
    // set error
    Double_t theError(0);
	...
	((RooRealVar*)theFVar)->rar_setRange(min, max);
...
void rarConfig::setColLimits(RooDataSet *data, Bool_t setLimits)
{
  ...
  for (Int_t i=0; i<nAddOns; i++) {
    ...
    if (setLimits) {
      Double_t theMin=fVar->rar_getMin();
      Double_t theMax=fVar->rar_getMax();
      Double_t dMin, dMax;
	...
	theMax=dMax;
      ...
      theVar->rar_setRange(theMin, theMax);
    } else
      theVar->removeRange();
    //cout<<" Derived obs "<<theName <<" set to ("<<theVar->rar_getMin()<<", "
    //<<theVar->rar_getMax()<<")"<<endl;
  }
}
			   ...
			   Bool_t setLimits)
{
  ...
  if (!addOns) return;
  // add the columns
  if (addColmns) data->addColumns(*addOns);
...
RooDataSet *rarConfig::createDataSet(const char *dsStr, Bool_t &isUB)
{
  isUB=kFALSE;
  ...
  if ("ascii"==dsType) { // dealing with ascii text file method
    if (1==datasetStrParser.nArgs()) { // #1
      data=RooDataSet::read(datasetStrParser[0], *getPrimaryObs());
    ...
    TObject *o=f.Get(datasetStrParser[1]);
    if (!o) {
      cout<< "Can not find "<<datasetStrParser[1]
	  <<" in file "<<datasetStrParser[0]<<endl;
      exit(-1);
    }
    TTree *t=dynamic_cast<TTree*>(o);
    RooDataSet *d=dynamic_cast<RooDataSet*>(o);
    if (d) {
      t=(TTree*)d->tree();
    }
    if (!t) {
    ...
    data=new RooDataSet("theData", "theData", t, *getPrimaryObs(), cut);
    addColumns(data);
    //data->Print("v");
