 ...
 * Package: RooRarFit
 *    File: $Id: rarBinned.cc,v 1.2 2007/06/29 08:37:35 zhanglei Exp $
 * Authors: Lei Zhang
 * History:
 * 
...
#include "RooRarFit/RooBinnedPdf.hh"
...
void rarBinned::init()
{
  ...
  if (_nBins<=0) _nBins=x->rar_getBins(); // get bins from its obs
  ...
  if (_nBins+1==nLimits) {
    ...
    Double_t binned=(x->rar_getMax() - x->rar_getMin())/_nBins;
    for (Int_t i=0; i<_nBins+1; i++) {
      (*_limits)[i]=x->rar_getMin()+i*binned;
    }
  }
  // set lo and hi limits
  //(*_limits)[0]=x->rar_getMin();
  //(*_limits)[_nBins]=x->rar_getMax();
  // read in coeffs
  for (Int_t i=0; i<_nBins-1; i++) {
