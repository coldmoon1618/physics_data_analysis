 ...
 * Package: RooRarFit
 *    File: $Id: rarBasePdf.cc,v 1.59 2010/06/15 13:09:57 fwilson Exp $
 * Authors: Lei Zhang
 * History:
 * 
                                 ...
                                 const Char_t *sec, Int_t *nBins)
{
  ...
  min=theVar->rar_getMin();
  max=theVar->rar_getMax();
  ...
  if (rangeParser.nArgs()>0) {
    ...
    rangeParser.Remove();
    if (min<theVar->rar_getMin()) min=theVar->rar_getMin();
  }
  if (rangeParser.nArgs()>0) {
    max=atof(rangeParser[rangeParser.nArgs()-1]);
    rangeParser.Remove(rangeParser.nArgs()-1);
    if (max>theVar->rar_getMax()) max=theVar->rar_getMax();
  }
  if (max<min) {
    Double_t v=max;
    max=min;
...
Double_t rarBasePdf::getCorrCoeff(TString pn1, TString pn2)
{
...
TString rarBasePdf::getCorrCoefName(TString pn1, TString pn2)
{
...
void rarBasePdf::doPdfFit(TString pdfList)
{
  ...
  Int_t pdfFitNumCPU=atoi(readConfStr("pdfFitNumCPU", "1", _runSec));
  ...
  for (Int_t i=0; i<fObsList.getSize(); i++) {
    ...
    if (!theObs) continue;
    Double_t fitMin=theObs->rar_getMin();
    Double_t fitMax=theObs->rar_getMax();
    getRange(theObs, "fitRange_", fitMin, fitMax);
    theObs->rar_setRange(fitMin, fitMax);
  }
...
RooPlot *rarBasePdf::doPdfPlot(TList &plotList, TString pdfList)
{
  ...
  for (Int_t i=0; i<fObsList.getSize(); i++) {
    ...
    Int_t nBins=atoi(readConfStr(Form("plotBins_%s", theObs->GetName()),
				 nBinStr, getVarSec()));
    if (nBins<0) continue;
    if (0==nBins) nBins=theObs->rar_getBins();
    // plot range
    Double_t plotMin=theObs->rar_getMin();
    Double_t plotMax=theObs->rar_getMax();
    ...
    TString rangeCuts="1";
    if (plotMin>theObs->rar_getMin())
      rangeCuts+=Form("&&(%s>%f)",theObs->GetName(), plotMin);
    if (plotMax<theObs->rar_getMax())
    ...
    theObs->rar_setRange(plotMin, plotMax);
				    ...
				    Double_t xmin,Double_t xmax, Double_t ymax)
{
  ...
  text->SetNDC(true); // just to get rid of the warning when compiling
  pIter->Reset() ;
  Double_t y=1-.4*step;
