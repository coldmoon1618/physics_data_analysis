// Make PDF plots for ksrg (4 rows, with BBbar)
//
...
void pdfplot_ksrg()
{
  ...
  if (doMass) {
    ...
    plotIt(mOmega_sig, ++iP, 6, .35, .15, .61);

    RooPlot* mOmega_bkg = f->Get("mOmega_the_mOmegaBkg");
    plotIt(mOmega_bkg, ++iP, 4, .30, .15, .70);
  ...
  ////////////////////// heli //////////////////////////////////
    ...
    plotIt(helomega_sig, ++iP, 3, .28, .15, .75);

    RooPlot* helomega_bkgsg = f->Get("heli_the_helimassBkg");
    plotIt(helomega_bkgsg, ++iP, 5, .28, .15, .30);
  }
  ...
  if (twopage) {
    plotIt2(deltaT_bkg, ++iP, 7, .30, .62, .57);
    if (doFeed) {
    ...
    plotIt(deltaT_bkg, ++iP,  7, .30, .62, .57);
    if (doFeed) {
      RooPlot* deltaT_feed = f->Get("deltaT_the_dtChls");
