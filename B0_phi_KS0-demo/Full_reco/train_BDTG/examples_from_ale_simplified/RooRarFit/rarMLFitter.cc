 ...
 * Package: RooRarFit
 *    File: $Id: rarMLFitter.cc,v 1.129 2010/06/15 13:09:59 fwilson Exp $
 * Authors: Lei Zhang
 * History:
 * 
...
#include "RooRarFit/rarMinuit.hh"
#include "RooRarFit/rarMLPdf.hh"
#include "RooRarFit/rarNLL.hh"
#include "RooRarFit/rarSPlot.hh"
...
void rarMLFitter::init()
{
  ...
  { // get protDataEVars defined in config file
    ...
    allProtEVars.
      add(getArgSet(readConfStr("protDataEVars","",_runSec),kFALSE));
				   ...
				   TString opt, ostream &o, Int_t ncpus)
{
  ...
  // output the time
  o<<endl<<"The mlFit time RealTime= " << timer.RealTime() << " CpuTime= "<< timer.CpuTime() << endl;
  // output the result
  o<<endl<<"The mlFit results (wrt "<<mlFitData->GetName()<<"):"<<endl;
  fitResult->Print();
...
RooDataSet *rarMLFitter::doToyStudy(RooArgSet fullParams)
{
  ...
  if (protDataStrParser.nArgs()>0) {
    ...
    if (!_protDataset) {
      cout<<" Error: Can not find dataset named "<<mpdsName<<endl
    ...
    cout<<" Error: No prototype datasets defined!!"<<endl
	<<" You could have problem with your toy study"<<endl
	<<" See config `protDatasets' in online doc for more info"<<endl;
    exit(-1);
  }
  cout<<" Using "<<_protDataset->GetName()<<" as master protDataset"<<endl;
  ...
  cout<<"Total Number of events: "<<toyNevt<<endl;
  cout<<"to be generated ";
  ...
  for (Int_t i=0; i<nComp; i++) {
    ...
    for (Int_t j=0; j<thisCompCoeffList.getSize(); j++) {
      RooArgList thisCompCoeffParams
	(*(thisCompCoeffList[j].getParameters(*_fullObs)));
  ...
  for (Int_t i=0; i<nCoeffParam; i++) {
    ...
    if (!coeffParamVar->rar_inRange(coeffParamVal, "range")) {
      cout<<coeffParamVal<<" not within the range of "
	  <<coeffParamVar->GetName()<<": ("
	  <<coeffParamVar->rar_getMin()<<","
	  <<coeffParamVar->rar_getMax()<<")"<<endl;
      exit(-1);
    }
  ...
  { // now, let's figure out what need to be adjusted
    ...
    for (Int_t i=0; i<nEvtType; i++) {
      ...
      if(0==evtTypeVarVal) {
	if (zEvtIdx!=-1) {
	  cout<<"Can not have more than one coeff=0"<<endl;
	  exit(-1);
	} else {
    ...
    cout<<"zEvtName "<<zEvtName<<" lEvtName "<<lEvtName<<endl;
	...
	if (!compCoeffSet.find("theSim_"+rarStrParser(zEvtName)[0])) {//simuPdf
	  // do not allow this
	  cout<<zEvtName<<" is set to zero but is not adjustable"<<endl;
	  exit(-1);
	  break;
	}
      }
	...
	if (!compCoeffSet.find("theSim_"+rarStrParser(lEvtName)[0])) {
	  // warning about this
	  cout<<lEvtName<<" is the largest but might not be adjustable"<<endl;
	}
      }
      lParamAdj=kTRUE;
      break;
    ...
    for (Int_t i=0; i<nCoeff; i++) {
      totEvt+=((RooAbsReal&)compCoeffList[i]).getVal();
    }
    if (totEvt!=toyNevt) {
	...
	cout<<"None src number is adjustable while it is needed to adjust"
	    <<endl;
	exit(-1);
      }
	...
	coeffParamSet.find(rarStrParser(evtTypeVar->GetName())[0]);
      if (!coeffAdjust) {
	cout<<" Can not find var "<<rarStrParser(evtTypeVar->GetName())[0]
	    <<endl;
	exit(-1);
      }
      nAdjust+=coeffAdjust->getVal();
      cout<<coeffAdjust->GetName()<<" needs to be adjusted from "
	  <<coeffAdjust->getVal()<<" to "<<nAdjust<<endl;
      if (!coeffAdjust->rar_inRange(nAdjust, "range")) {
	cout<<nAdjust<<" not within the range of "<<coeffAdjust->GetName()
	    <<": ("<<coeffAdjust->rar_getMin()<<","
	    <<coeffAdjust->rar_getMax()<<")"<<endl;
	exit(-1);
      }
	...
	if (fabs(totEvt-toyNevt)>.01) {
	  cout<<endl<<" Adjusting "<<evtTypeVar->GetName()
  ...
  Int_t toyFitNumCPU=atoi(readConfStr("toyFitNumCPU", "1", _runSec));
  ...
  // print full obs
  cout<<"full obs for toy study"<<endl;
  _fObsSet.Print("v");
  
  // get protGen
  if (!getProtGen()) {
    cout<<" Can not create protGen!"<<endl;
    exit(-1);
  }
  ...
  { // let's find fullProtVars
    ...
    cout<<"Full protDataVars"<<endl;
    fullProtVars.Print("v");
    cout<<" protGenLevel: "<<_protGenLevel<<endl;
  }
  
  if (!getGenerator()) {
    cout<<" Can not create toy Gen!"<<endl;
    exit(-1);
  }
  ...
  if (1==_protGenLevel) { // get default protData
    ...
    cout<<"The prototype dataset for toy study:"<<endl;
    protData->Print("v");
  ...
  for (Int_t expIdx=0; expIdx<nLoops; expIdx++) {
	...
	if (!theParam) {
	  cout<<"Can not find param "<<preToyRandParser[2*i]<<endl;
	  continue;
	}
	...
	if (!theParam) {
	  cout<<"Can not find cloned param "<<theParam->GetName()<<endl;
	  exit(-1);
	}
	cParams.add(*theParam);
	cout<<theParam->GetName()<<" = "<<theParam->getVal();
    ...
    if (""!=genStr) {
      cout<<"Using generation string: \""<<genStr<<"\" for embedding"<<endl;
    }
	...
	delete fCatData;
      ...
      cout<<"protData from protGen:"<<endl;
      protData->Print("v");
      //protData->write("/tmp/pd.text");
    ...
    if (firstToy) {
      cout<<"The toyDeps"<<endl;
      toyDeps.Print("v");
      cout<<"The protVars"<<endl;
      _protDataVars.Print("v");
    }
    ...
    if ("no"!=readConfStr("toyGenerate", "yes", _runSec)) {
      if (firstToy) {
	cout<<endl<<" RooMCStudy params for generating:"<<endl;
	...
	if (firstToy) {
	  cout<<"etoyDeps"<<endl;
	  etoyDeps.Print("v");
	  cout<<"protDataEVars"<<endl;
	  _protDataEVars.Print("v");
	  cout<<"genStr "<<genStr<<endl;
	}
	...
	_datasets->ubStr(_datasets->getDSName("toySample"), (Char_t*)"Unblinded");
      }
    }
    // fit
	...
	coeffParamSet.Print("v");
      ...
      if (toyChkNegativePdf&&chkNPdfDS) {
        ...
        for (Int_t iChkEvt=0; iChkEvt<nChkEvt; iChkEvt++) {
          ...
          if (isNegativeValue()) {
            cout<<"The PDF have negative value for current event"<<endl;
            theEvt->Print();
            theEvt->Print("v");
            exit(-1);
          }
      ...
      if (""!=genStr) {
	cout<<"Re-calculate pulls for embedded toys"<<endl;
	...
	if (fr->status()) {
	  cout<<" Fit status for experiment #"<<expIdx<<"-"<<nExpPerLoop-i
	      <<": "<<fr->status()<<endl;
	  continue;
	}
...
void rarMLFitter::doLLRPlot(RooDataSet *projData, TList &plotList)
{
  ...
  // construct lRatio
  const RooAbsReal *theSProj=_theSPdf->rar_createProjection(depVarSet, projVarSet);
  const RooAbsReal *theBProj=_theBPdf->rar_createProjection(depVarSet, projVarSet);
				 ...
				 TList &plotList)
{
  ...
  if ("-1"==LRatioCut) LRatioCut=readConfStr("projLRatioCut", ".90", _runSec);
  ...
  if ("no"!=findOptCut) {
   ...
   if (("1"!=optRangeCut)||("1"!=ppdCut)) {
    ...
    for (Int_t i=0; i<_sCompList.GetSize(); i++) {
      ...
      RooDataSet *theData=rarPdf->getData();
  ...
  // reduce the dataset
  RooDataSet *sliceData=(RooDataSet*)projPlotData->reduce(LRatioCut);
  // get plot bins
  Int_t nBins=atoi(readConfStr(Form("plotBins_%s", theVar->GetName()),
			       "0", _runSec));
  if (nBins<=0) nBins=theVar->rar_getBins();
  // get X error scaling
  Double_t xerrorscale = atof(readConfStr("XErrorSize", "1.0", _runSec));
  // plot range
  Double_t plotMin=theVar->rar_getMin();
  Double_t plotMax=theVar->rar_getMax();
  RooBinning *abins=
  ...
  if ("no"==saveDS) delete projPlotData;
  else {
    TString dsName=projPlotData->GetName();
    dsName+="_";
    dsName+=theVar->GetName();
    projPlotData->SetName(dsName);
    TTree *theDSTree=(TTree*)projPlotData->tree();
    theDSTree->SetName(dsName);
				  ...
				  RooPlot *frameM, RooPlot *frameP)
{
  ...
  // ag: set at least one floating parameter to the pdf and each component
  // of the pdf's. Otherwise a constant pdf will be plotted.
  // This is not the most elegant workaround, but it does what I want...

  RooArgSet *thePdfComps = _thePdf->getComponents();
  TIterator *iter = thePdfComps->createIterator();
  RooAbsPdf *myPdf(0);

  while ( myPdf = (RooAbsPdf*)iter->Next() ) {

    RooArgSet* paramSet = myPdf->getParameters(_protDataset) ;
    ...
    while((param = iter.next())) {
      count++ ;
      if ( count <= 1 ) continue;
      param->setAttribute("Constant", kFALSE);
      break;
    }
    delete paramSet ;
  }
  ...
  if (!(frameM&&frameP)) { // plot pdf and components
    ...
    // plot the total pdf (sum of the components):
...
RooPlot *rarMLFitter::doScanPlot(TList &plotList)
{
  ...
  while (scanVarsParser.nArgs()>0) {
    ...
    scanVars.add(*theVar);
    // new min?
    Double_t min=theVar->rar_getMin();
    Double_t max=theVar->rar_getMax();
  ...
  if (scanVars.getSize()==1) {
    ...
    frame=theVar->frame(theVar->rar_getMin(), theVar->rar_getMax(), 100);
  ...
  // save the TTree;
  TTree *theDSTree=(TTree*)theDS->tree();
  plotList.Add(theDSTree);
  // set max for frame
...
RooPlot *rarMLFitter::doContourPlot(TList &plotList)
{
  ...
  xMin=contX->rar_getMin();
  xMax=contX->rar_getMax();
  // check if the next two are limits for it
  ...
  yMin=contY->rar_getMin();
  yMax=contY->rar_getMax();
  // check if the next two are limits for it
  if (isNumber(contourVarsParser[0])) {
...
RooPlot *rarMLFitter::doSPlot(RooRealVar *theVar, TList &plotList)
{
  ...
  for (Int_t i=0; i<nModel; i++) {
	...
	cout<<" "<<theYield.GetName()<<" is constant, ignored in sPlot"<<endl;
        ...
        if (theYield.rar_getMin()>-1e6) {
          theYield.setMin(-1e6);
          cout<<" The lower limit of "<<theYield.GetName()
              <<" changed to -1e6"<<endl;
        }
        if (theYield.rar_getMax()<1e6) {
          theYield.setMax(1e6);
  ...
  for (Int_t i=0; i<yields.getSize(); i++) {
    ...
    if (("yes"==readConfStr("sPlotSaveSWeight", "no", _runSec))&&fillData) {
      ...
      TTree* dataTree = (TTree*) (fillData->tree());
      dataTree->SetName(dName);
...
void rarMLFitter::run()
{
  ...
  if ("no"!=readConfStr("toyStudy", "no", _runSec)) {
    ...
    if (("no"!=postToyWriteParams) && theFitParDataSet) {
	  ...
	  <<toyRootFile<<endl;
      TFile toyFile(toyRootFile, "recreate");

      // original:
      //((TTree*)theFitParDataSet->tree())->Write();
      // not sure why it returns the null pointer...

      theFitParDataSet->Write();
      toyFile.Close();
    }
  } // done toy
  ...
  if ("no"!=readConfStr("mlFit", "no", _runSec)) {
    ...
    // get number of cpus option
    Int_t mlFitNumCPU=atoi(readConfStr("mlFitNumCPU", "1", _runSec));
