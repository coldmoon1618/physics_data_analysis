...
void FitMbcBDTcut(){
    ...
    const char *fIn  [2] = {"Data_resp.root"        ,"Cand_resp.root"         };
    //const char *fOut [2] = {"Data_Mbc.pdf"          ,"MC_Mbc.pdf"           };
    const char *fOut [2] = {"Data_resp.pdf"         ,"MC_Mbc.pdf"           };
    const char *title[2] = {"M_{bc} (Data ~68/fb)"  ,"M_{bc} (MC13a 1/ab)"  };
    const int  nBins [2] = {20,35};
    const int  nPlot [2] = {17,30};
    //RooDataSet Mbc_data("Mbc_data","Mbc_data",Mbc, ImportFromFile(fIn[isMC],"B0_cand"),Cut("response")); 
    RooDataSet Mbc_data("Mbc_data","Mbc_data",RooArgSet(Mbc,response), ImportFromFile(fIn[isMC],"res_BDT"),Cut("response>0")); 
