...
void cutOnGeneric() {
    ...
    double  pstar;
    KS0s->SetBranchAddress("useCMSFrame__bop__bc",&pstar);
    ...
    //TFile fOut("proc11_exp8_nCand.root","recreate");
    TFile fOut("proc11_exp8_cut.root","recreate");
    //TFile fOut("MC13a_generic_cut.root","recreate");
    TTree *tree = new TTree("tree","");
    double mass;
    tree -> Branch("M",&mass,"M/D");

    for(int i=0; i<KEntries/50; i++)
    //for(int i=0; i<KEntries; i++)
    //for(int i=0; i<355797; i++)
    {
        KS0s -> GetEntry(i);
        if(pstar<2.2)continue;
