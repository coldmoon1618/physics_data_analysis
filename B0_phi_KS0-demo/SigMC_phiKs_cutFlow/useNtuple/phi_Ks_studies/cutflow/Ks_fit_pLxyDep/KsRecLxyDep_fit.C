...
void KsRecLxyDep_fit(){
    const int nTrees = 8;
    const char *treeName[nTrees] = {"tPre_LoLxy_ind",
                                    "tVtx_LoLxy_ind",
                                    "tGbk_LoLxy_ind",
                                    "tGbk_LoLxy_acc",
                                    "tPre_HiLxy_ind",
    ...
    for(int isMat = 0; isMat<2; isMat++){
        ...
        ofstream myfile;
        if(isMat)   myfile.open("KsMatLxyDep_fitResults.txt");
        else        myfile.open("KsRecLxyDep_fitResults.txt");

        TString fInName("");
        if(isMat)   fInName+="KsMatLxyDep_toFit.root";
        else        fInName+="KsRecLxyDep_toFit.root";
        ...
        if(isMat)   c_M -> SaveAs("KsMatLxyDep_fit.pdf");
        else        c_M -> SaveAs("KsRecLxyDep_fit.pdf");
    }
