...
##########################################################################
input_list = ['/group/belle2/users/tenchini/prerelease-05-00-00a/charged/charged_eph3_BGx0_0.root']
...
ma.matchMCTruth(list_name='B+:generic', path=path)

Btag_cuts = 'Mbc>5.2 and abs(deltaE)<0.5 and SigProb>0.01'
...
# DecayHash needed to be created before trying to access from extraInfo
...
path.add_module('ParticleMCDecayString', listName='B+:MC1',fileName='mc1Hash.root')
ma.cutAndCopyList('B+:MC2','B+','mdstIndex==2', path=path)
path.add_module('ParticleMCDecayString', listName='B+:MC2',fileName='mc2Hash.root')

ma.variablesToNtuple('B+:MC1',B_MC_vars,treename='B_plus', filename='mainOut.root',path=path)
ma.variablesToNtuple('B+:MC2',B_MC_vars,treename='B_minus', filename='mainOut.root',path=path)
##########################################################################
